...
unsigned char gSaveBmp=0;
unsigned char img1[2000*480*3];

typedef struct rbg_tag
{
	unsigned char b;
	unsigned char g;
	unsigned char r;
}t_rgbcell;
#define BMP_STRID 1536
//#define BMP_STRID (640*3)

static void LoadWinBmp()
{	
	int u=0,v=0,r=0,g=0,b=0;
	int rdif=0,invgdif=0,bdif=0;
	//unsigned int ypos=0,upos=0,vpos=0;
	t_rgbcell *pRBGcell =(t_rgbcell *)img1;

    //graphics overlay
	BITMAPINFO_unx* infoLoad;
	unsigned char *bitmap=NULL;
	//bitmap = LoadDIBitmap("D:/999_temp/test.bmp",&infoLoad);
	bitmap = LoadDIBitmap("D:/999_temp/testreal.bmp",&infoLoad);
	memset(img1,0,sizeof(img1));
	for(int i=0;i<240;i++)
	{
		for(int j=0,k=0;j<(640<<1),k<640;j+=4,k+=2)
		{
			//pixel-0

#if 1
			v = bitmap[i*BMP_STRID+j+1] - 128;
			u = bitmap[i*BMP_STRID+j+3] - 128;
			//v=0;
			//u=0;
#else
			v = bitmap[i*BMP_STRID+j+0] - 128;
			u = bitmap[i*BMP_STRID+j+2] - 128;
#endif
			rdif = v + ((v * 103) >> 8);
			invgdif = ((u * 88) >> 8) +((v * 183) >> 8);
			bdif = u +( (u*198) >> 8);
#if 1
			r = bitmap[i*BMP_STRID+j] + rdif;
			g = bitmap[i*BMP_STRID+j] - invgdif;
			b = bitmap[i*BMP_STRID+j] + bdif;
#else
			r = bitmap[i*BMP_STRID+j+1] + rdif;
			g = bitmap[i*BMP_STRID+j+1] - invgdif;
			b = bitmap[i*BMP_STRID+j+1] + bdif;
#endif
			if (r>255) r=255;
			if (r<0) r=0;
			if (b>255) b=255;
			...
			pRBGcell[i*640+k].r = r;	
			pRBGcell[i*640+k].g = g;	
			pRBGcell[i*640+k].b = b;	
#if 1
			//pixel-1
			r = bitmap[i*BMP_STRID+j+2] + rdif;
			g = bitmap[i*BMP_STRID+j+2] - invgdif;
			b = bitmap[i*BMP_STRID+j+2] + bdif;
#else
			r = bitmap[i*BMP_STRID+j+3] + rdif;
			g = bitmap[i*BMP_STRID+j+3] - invgdif;
			b = bitmap[i*BMP_STRID+j+3] + bdif;
#endif
			...
			pRBGcell[i*640+k+1].r = r;	
			pRBGcell[i*640+k+1].g = g;	
			pRBGcell[i*640+k+1].b = b;
		}	
	}

	//SaveDIBitmap("D:/999_temp/RvsCaptureSample01.bmp",infoLoad,bitmap);
	SaveDIBitmap("D:/999_temp/RvsCaptureSample01.bmp",infoLoad,img1);


    free(bitmap);
}
static void SaveWinBmp()
{
    //graphics overlay
    BITMAPINFO_unx info;
	memset(&info,0,sizeof(BITMAPINFO_unx));
    ...
    unsigned char *imgtmp =(unsigned char *)malloc(640*480*3);
    memset(imgtmp,0,640*480*3);
    unsigned char *pimg = imgtmp;
#if 0
...
//test
    for(int i =0;i<480;i++)
    {
        for(int j =0;j<640;j++)
        {
            ...
            if((j<320)&&(i<=240))
            {
                img[i*(640*3)+(3*j+0)] = 60;
				img[i*(640*3)+(3*j+1)] = 110;
				img[i*(640*3)+(3*j+2)] = 30;
            }
            else if((j<320)&&(i>240))
            {
                img[i*(640*3)+(3*j+0)] = 160;
				img[i*(640*3)+(3*j+1)] = 20;
				img[i*(640*3)+(3*j+2)] = 100;
            }
            else if((j>=320)&&(i<=240))
            {
                img[i*(640*3)+(3*j+0)] = 60;
				img[i*(640*3)+(3*j+1)] = 50;
				img[i*(640*3)+(3*j+2)] = 110;
            }
            else if((j>=320)&&(i>240))
            {
                img[i*(640*3)+(3*j+0)] = 160;
				img[i*(640*3)+(3*j+1)] = 120;
				img[i*(640*3)+(3*j+2)] = 100;
            }
#endif


            *pimg = img1[i*(640*3)+3*j];
            pimg++;
            *pimg = img1[i*(640*3)+(3*j+1)];
            pimg++;
            *pimg = img1[i*(640*3)+(3*j+2)];
            pimg++;
        }
    }

    SaveDIBitmap("D:/999_temp/RvsCaptureSample.bmp",&info,imgtmp);
    free(imgtmp);
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, TCHAR *lpCmdLine, int nCmdShow)
{
	...
	*/
	...
	EGLint i;
	eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SAMPLE_BUFFERS, &i);
	printf("EGL_SAMPLE_BUFFERS--------------------->%d\n",i);
    eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SAMPLES, &i);
	printf("EGL_SAMPLES--------------------->%d\n",i);
	eglGetConfigAttrib(eglDisplay, eglConfig, EGL_RENDERABLE_TYPE, &i);
