...
#define VDT_TEX_GREEN_DAY 0
#define VDT_TEX_BLUE_DAY 0
#define VDT_TEX_ALPHA_DAY 255

#define VDT_TEX_RED_NIGHT  255
#define VDT_TEX_GREEN_NIGHT 255
#define VDT_TEX_BLUE_NIGHT 255
#define VDT_TEX_ALPHA_NIGHT 255
...
GraphicVDT::GraphicVDT(void)
{
    ...
    for (int i=0;i<4;i++)
    {
        for (int j=0;j<4;j++)
        {
            m_linetex[i*4*4+j*4] = VDT_TEX_RED_DAY;
            m_linetex[i*4*4+j*4+1] = VDT_TEX_GREEN_DAY;
            m_linetex[i*4*4+j*4+2] = VDT_TEX_BLUE_DAY;
            m_linetex[i*4*4+j*4+3] = VDT_TEX_ALPHA_DAY;
        }
    }
	//default
	...
	m_CarPOT.ftheta_long =R_PI/2; //R_PI/2;
...
void GraphicVDT::vSetStyle(eLineStyle DayNight)
{
    ...
    {
        eTexRed = VDT_TEX_RED_DAY;
        eTexGreen = VDT_TEX_GREEN_DAY;
        eTexBlue = VDT_TEX_BLUE_DAY;
        eTexAlpha = VDT_TEX_ALPHA_DAY;
    }
    else
    {
        eTexRed = VDT_TEX_RED_NIGHT;
        eTexGreen = VDT_TEX_GREEN_NIGHT;
        eTexBlue = VDT_TEX_BLUE_NIGHT;
        eTexAlpha = VDT_TEX_ALPHA_NIGHT;
    }
    for (int i=0;i<4;i++)
    {
        for (int j=0;j<4;j++)
        {
            m_linetex[i*4*4+j*4] = eTexRed;
            m_linetex[i*4*4+j*4+1] = eTexGreen;
            m_linetex[i*4*4+j*4+2] = eTexBlue;
            m_linetex[i*4*4+j*4+3] = eTexAlpha;
        }
    }
}
...
void GraphicVDT:: CalLineData()
{
    ...
    */
    ...
    {
        PDCA_LOG_INFO("GraphicVDT e_Rearward update shortR[%f] longR[%f]\n",m_CarPOT.shortsemi_circle_radius,m_CarPOT.long_semi_circle_radius);
        for (int j=0;j<LINE_PTN_NUM/2;j++)
        {
    ...
    else if (line_status==e_Forward_Lt)//left turn is negative turn, and short leftR long rightR
    {
		...
		PDCA_LOG_INFO("GraphicVDT e_Forward_Lt update shortR[%f] longR[%f]\n",m_CarPOT.shortsemi_circle_radius,m_CarPOT.long_semi_circle_radius);
        for (int j=0;j<LINE_PTN_NUM/2;j++)
        {
    ...
    //PDCA_LOG_INFO("GraphicVDT update shortR[%d] longR[%d]\n",m_CarPOT.shortsemi_circle_radius,m_CarPOT.long_semi_circle_radius);
}
...
{
    static int lastvdtDir = -1;
    static int lastvdtStatus = -1;
    static int lastshortRadius = -1;
    static int lastlongRadius = -1;
    if((lastvdtDir == vdtDir)
    ...
    //plus offset for car --> unknow the input radius equals how many pixcels??
	shortRadius*=1.22;
	longRadius*=1.22;
