...
BOOLEAN	CAdasCtrl::canManualDeactivateCamera1()
{
 ...
 //2017-5-26 special case: last manually active camera 2->auto active cam1->manually deactivate override cam1->hide both cam	
	 BOOLEAN bDeact = m_pCameraCtrl->canManualDeactivate(CCamera::eCamera1);
		if(bDeact)
		{
			if(NULL!=m_pCameraCtrl->getCamera(CCamera::eCamera2))
			{
				if(m_pCameraCtrl->getCamera(CCamera::eCamera2)->isLastManualActivated())
				m_pCameraCtrl->getCamera(CCamera::eCamera2)->ResetLastManualActivated();

				if(m_pCameraCtrl->getCamera(CCamera::eCamera2)->isManualActivated())
				{
					m_pCameraCtrl->getCamera(CCamera::eCamera2)->SetStateDeactivate();
					DBG_MSG(("received hide camera1 manually and found camera2 last manually activated, cancel camera2 LMA state now\n"));
				}
			}
		}
	return bDeact;
...
{
	//if current is standby ignore this request
	TRC_SCOPE(ADAS, CAdasCtrl, canManualDeactivateCamera2);
	if(CScaniaOnOffClient::getInstance()->isInStandbyState())
	{
	...
	//2017-5-26 special case: last manually active camera 1->auto active cam2->manually deactivate override cam2->hide both cam   
	BOOLEAN bDeact = m_pCameraCtrl->canManualDeactivate(CCamera::eCamera2);
	if(bDeact) 
	{
		if(NULL!=m_pCameraCtrl->getCamera(CCamera::eCamera1))
		{
			if(m_pCameraCtrl->getCamera(CCamera::eCamera1)->isLastManualActivated())
			m_pCameraCtrl->getCamera(CCamera::eCamera1)->ResetLastManualActivated();

			if(m_pCameraCtrl->getCamera(CCamera::eCamera1)->isManualActivated())
			{
				m_pCameraCtrl->getCamera(CCamera::eCamera1)->SetStateDeactivate();
				DBG_MSG(("received hide camera2 manually and found camera1 last manually activated, cancel camera1 LMA state now\n"));
			}
		}
	}
	return bDeact;
