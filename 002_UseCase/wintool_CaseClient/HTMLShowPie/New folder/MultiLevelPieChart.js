(function(){
	...
	Chart.Type.extend({
		...
		defaults: {
			...
			// String - The colour of each segment stroke red/green/blue
		...
		addSegments: function(segmentData, depth, collection){
			...
			if (this.options.skipDirectChildren && segmentData.children && segmentData.children.length == 1) {
				...
				var segment = new this.SegmentArc({
					...
					//------------------------add rate info in data structure
					Parents : segmentData.Parents,
					FValue : segmentData.FValue,
					SValue : segmentData.SValue,
					TValue : segmentData.TValue,
