...
(function(){
	...
	Chart.defaults = {
		global: {
			...
			tooltipTemplate: "<%if (label){%><%=label%>#<%}%><%%><%if(FValue>=0){%>Total/Target : <%=FValue%>/<%=value%><%}%><%if (TValue){%>Passed/Total: <%=value%>/<%=TValue%><%}%><%if (SValue){%>Total: <%=value%><%}%>",// <%if (SValue){%>TotalNumber: <%=SValue%<%}%>" ,
			//tooltipTemplate: "<%= label %>#<%= value %> , Rate <%= rate %>",
...
///////////////////////////////////////////////////////
	Chart.Tooltip = Chart.Element.extend({
		draw : function(){
			...
			var tooltipWidth = ctx.measureText(this.text).width/2 + 2*this.xPadding+ 100,
			...
			ctx.fillText(arr[0], tooltipX + 10, tooltipY + 15);
			ctx.fillText(arr[1], tooltipX + 10, tooltipY + tooltipRectHeight/2+ 10);
		}
	});
	...
	Chart.MultiTooltip = Chart.Element.extend({
		...
		draw : function(){
			...
			var arr=  this.text.split("#");
			ctx.fillText(arr[0], tooltipX + 10, tooltipY + 15);
			ctx.fillText("Count: "+arr[1], tooltipX + 10, tooltipY + tooltipRectHeight/2+ 10);ctx.fillText(this.title,this.x + this.xPadding, this.getLineHeight(0));
