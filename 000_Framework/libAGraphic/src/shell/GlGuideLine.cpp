...
void GlGuideLine::SetDistLineParam(float startDis,float deltaDis,int MarklineNum)
{
	...
	for(int i=1;i<(m_iGroupNum);i++)
	{
		...
		for(j=0;j<MarklineNum;j++)
		{
			pLineInfo->dist[j] = startDis + j*deltaDis;
		}
		startDis=pLineInfo->dist[j-1]+MarklineNum*deltaDis;
	}
    //last dis----should match with side length, refactor later....tbd
    pLineInfo = &m_pDistLineInfo[m_iGroupNum-1];
    pLineInfo->dist[0] =RVS_GUIDELINE_FAR_END_DISTANCE;
...
void GlGuideLine::SetSideLineParam(float* sepDis,float* LSegExtendLength)
{
    ...
    //last seg
    //pLineInfo = &m_pSideLineInfo[m_iGroupNum-1];
    //pLineInfo->endDis = RVS_GUIDELINE_FAR_END_DISTANCE;
}
