...
LoadDIBitmap(const char *filename, /* I - File to load */
    ...
    int              infosize;     /* Size of header information */
    BITMAPFILEHEADER_unx header;       /* File header */
    ...
    if ((*info = (BITMAPINFO_unx *)malloc(sizeof(BITMAPINFO_unx))) == NULL)
        {
        /* Couldn't allocate memory for bitmap info - return NULL... */
        fclose(fp);
        return (NULL);
...
SaveDIBitmap(const char *filename, /* I - File to load */
    ...
    /* Figure out the header size */
    infosize = sizeof(BITMAPINFOHEADER_unx);
    switch (info->bmiHeader.biCompression)
	{
	case BI_BITFIELDS :
            infosize += 12; /* Add 3 RGB doubleword masks */
    ...
    size = sizeof(BITMAPFILEHEADER_unx) + infosize + bitsize;
